 ...
 * @written Feb 8, 2011 -------------------------------------------------------
 */
public class RobotPotentiometer extends AnalogInput implements Potentiometer {
	/**
	 * ------------------------------------------------------
	 *
	 ...
	 * a channel and the module passed in.
	 *
	 * @method Potentiometer() - constructor
	 * @param channel
	 *            - the port for the digital input
	 ...
	 */
	/*
	 * public RobotPotentiometer ( final int slot, final int channel) {
	 * super(slot, channel); this.maxDegreesForPotentiometer =
	 * kDEFAULT_MAX_DEGREES; } // end RobotPotentiometer
	 */
	// -------------------------------------------------------
	/**
	 * Create an instance of a LightSensor class. Creates a digital input given
	 ...
	 * this.maxDegreesForPotentiometer = maxDegrees; } // end RobotPotentiometer
	 */
	// -------------------------------------------------------
	/**
	 ...
	 *
	 * @param range
	 *            Total range to use ex. range of 100 corresponds to -50 to +50
	...
	public double get(double range) {
		...
		final double retVal = (((2 * this.get()) - this.maxDegreesForPotentiometer) / this.maxDegreesForPotentiometer)
				* (range / 2);
		if (this.isReversed == false)
			return retVal;
	...
	public double get(double minRange, double maxRange) {
		...
		final double retVal = ((this.getValue() / kTOTAL_RAW_UNITS) * tempRange) + minRange;
		if (this.isReversed == false)
			return retVal;
		else
			// If the potentiometer is reversed, negate our position
	...
	// -------------------------------------------------
	/**
	 * Gets the location within the range using our max degree value as the
	 * range.
	 *
	 * @return location in the range
	 * @author Noah Golmant
	...
	public double getFromRange() {
		// (percent off from middle) * (range of values)
		final double retVal = (((2 * this.get()) - this.getMaxDegrees()) / this.getMaxDegrees())
